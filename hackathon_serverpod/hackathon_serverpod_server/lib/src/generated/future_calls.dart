...
import '../tasks/task_vote_future_call.dart' as _infu6136;
...
class FutureCalls extends _is.FutureCallDispatch<_FutureCallRef> {
  ...
  void initialize(
    ...
    var registeredFutureCalls = <String, _is.InvokableFutureCall>{
      'TaskVoteExpireVoteFutureCall': TaskVoteExpireVoteFutureCall(),
    };
...
class _FutureCallRef {
  ...
  late final taskVote = _TaskVoteFutureCallDispatcher(_invokeFutureCall);
}

class _TaskVoteFutureCallDispatcher {
  _TaskVoteFutureCallDispatcher(this._invokeFutureCall);
  ...
  Future<void> expireVote(
    ...
    var object = _if11z8io.TaskVoteFutureCallExpireVoteModel(
      taskId: taskId,
      expectedVoteClosesAt: expectedVoteClosesAt,
    );
    return _invokeFutureCall(
      'TaskVoteExpireVoteFutureCall',
      object,
    );
  }
}

class TaskVoteExpireVoteFutureCall
    extends _is.FutureCall<_if11z8io.TaskVoteFutureCallExpireVoteModel>
    implements
        _is.InvokableFutureCall<_if11z8io.TaskVoteFutureCallExpireVoteModel> {
  @override
  _ida.Future<void> invoke(
    _is.Session session,
    _if11z8io.TaskVoteFutureCallExpireVoteModel? object,
  ) async {
    if (object != null) {
      await _infu6136.TaskVoteFutureCall().expireVote(
        session,
        object.taskId,
        object.expectedVoteClosesAt,
      );
    }
  }
}
